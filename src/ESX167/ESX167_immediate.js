...
function addContent() {
  if (utag_data.site_type == "desktop") {
    ...
    content = `
      <style> #ChatIcon { display: flex; flex-wrap: no-wrap; position: absolute; top: 0; left: 50%; transform: translateY(-25%); } #ChatIcon > * { display: inline-block; } #ChatIcon img { zoom: 50%; cursor: pointer; } @media (max-width: 1199px) { #ChatIcon { left: 55%; margin-right: 1rem; } } </style> <div id="ChatIcon"> <img class="img-responsive" src="https://www.livingspaces.com/globalassets/images/home/2020/04/chat-delivery-desktopx2.png" /> <span id="RemoveRepChat" style="font-size: 4rem; color: #333; margin-top: 3%; cursor:pointer; transform: translateY(-20.2px)" aria-hidden="true" >×</span > </div>
    `;

    // inject
    desktopInsertHook.insertAdjacentHTML("afterend", content);
  } else {
    const mobileInsertHook = document.getElementById("delivery-options");
