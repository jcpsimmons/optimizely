function ESX081() {
  ...
  $(".blog-navigation").after(
    '<div class="container"><div class="centerblock"> <div class="search_boxWrapper"> <input autocomplete="off" type="text" name="article_search" id="article_search" placeholder="Search ideas + advice articles" class="form-control" /></div> </div> <ul class="list-group" id="result"></ul> <br /> </div>'
  );
  ...
  $("#article_search")
    .keyup(function() {
      ...
      if (suggestionTitle !== null) {
        var loopLength =
          suggestionTitle.length > 4 ? 4 : suggestionTitle.length;
        for (var i = 0; i < loopLength; i++) {
          if (
            $(".article_li:contains(" + suggestionTitle[i][1] + ")").length > 0
          ) {
          ...
          $("#result").prepend(
            '<li class="list-group-item link-class" onclick="search_link()"><a style="display:block;" href="' +
              articles_search[suggestionTitle[i][1]] +
              '">' +
              suggestionTitle[i][1] +
              "</a></li>"
          );
  ...
  function getArticleList() {
    ...
    $.each(articles_search, function(key, value) {
      if (
        key.toString().search(expression) != -1 ||
        value.search(expression) != -1
      ) {
        $("#result").append(
          '<li class="list-group-item link-class article_li" onclick="search_link()"><a class="articleLinkTag" style="display:block;" href="' +
            ...
            '"> ' +
            key.toString() +
            ' <span class="rangle">&rsaquo;</span></a></li>'
        );
      }
    });
  }
  ...
  $("body").on("touchstart", function(e) {
    ...
    if (
      ...
      e.target.id != subject2.attr("id") &&
      !$(e.target).hasClass("articleLinkTag")
    ) {
      setTimeout(function() {
        $("#result").html("");
        $("#state").val("");
