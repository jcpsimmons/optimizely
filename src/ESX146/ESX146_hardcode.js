...
// Get a list of all the 2 day and 5 day zips
const twoDayZips = [zipCodes.california.bayArea.codes, zipCodes.texas.austin.codes, zipCodes.texas.dallas.codes].flat()
const fiveDayZips = [zipCodes.california.full.codes, zipCodes.arizona.full.codes].flat()

// Experiment Code

function checkAndUpdate() {
    // Create a method for calculating days in the future on constructor
    Date.prototype.addDays = function (days) {
        this.setDate(this.getDate() + parseInt(days));
        return this;
    };

    var firstDeliveryDate = Date.parse(
        window.server.DeliveryDates[0].value.split(" ")[0]
    );

    var today = new Date();
    var twoDaysOut = today.addDays(2)
    ...
    if (
        (twoDayZips.includes(curZip) && firstDeliveryDate < twoDaysOut) ||
        (fiveDayZips.includes(curZip) && firstDeliveryDate < fiveDaysOut)
    ) {
        //   ADD COLON if it doesnt have one. Various styling
        if (
            $("p.delivery-fee")
                .html()
                .search(":") == -1
        ...
        for (var i = 0; i < stateNames.length; i++) {
            ...
            for (var t = 0; t < areaNames.length; t++) {
                ...
                if (curItem.codes.includes(curZip)) {
                    $('body').append('<style> p.delivery-fee::after { content: "Shipping From ' + curItem.location + '"; position: absolute; right: 0; bottom: 1px } </style>')
                    break
                }
            }
        }
