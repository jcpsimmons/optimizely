...
function checkAndUpdate() {
  ...
  var firstDate = Date.parse(window.server.DeliveryDates[0].value.split(' ')[0]);
  var today = new Date();
  ...
  if (state == 'CA' && firstDate < fiveDaysOut || state == 'AZ' && firstDate < fiveDaysOut) {
    ...
    if (state == 'CA') {
      $('body').append('<style> p.delivery-fee::after { content: "Shipping From Rialto, CA 92324"; position: absolute; right: 0; bottom: 1px } </style>');
    } else {
