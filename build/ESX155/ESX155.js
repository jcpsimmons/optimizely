var handlerFunction = function handlerFunction() {
  ...
  var desktopSite = window.utag_data.site_type == 'desktop';

  var viewYourOptions = function () {
    if (desktopSite) {
      return document.querySelector('.search-form-container button');
    }
    ...
    for (var i = 0; i < stateNames.length; i++) {
      ...
      setTimeout(function () {
        if (!document.getElementById('ShippingFromContainer')) {
          if (place) {
            injectionPoint.parentNode.before(makeNode("<div class=\"col-xs-12\" id=\"ShippingFromContainer\" style=\"".concat(function () {
              if (desktopSite) {
                return "margin-top:2rem;";
              }

              return "text-align:center";
            }(), "\"><span style=\"font-size:").concat(function () {
              if (desktopSite) {
                return "1.6rem";
              }

              return "1.5rem";
            }(), ";\">Ships from: ").concat(place, "</span></div>")));
          }
        } else {
          if (place) {
            document.querySelector('#ShippingFromContainer span').textContent = "Ships from: ".concat(place);
