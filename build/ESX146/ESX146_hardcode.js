...
var twoDayZips = [zipCodes.california.bayArea.codes, zipCodes.texas.austin.codes, zipCodes.texas.dallas.codes].flat();
var fiveDayZips = [zipCodes.california.full.codes, zipCodes.arizona.full.codes].flat();
...
function checkAndUpdate() {
  ...
  var firstDeliveryDate = Date.parse(window.server.DeliveryDates[0].value.split(" ")[0]);
  var today = new Date();
  var twoDaysOut = today.addDays(2);
  var fiveDaysOut = today.addDays(5);
  var curZip = parseInt($(".current-shipping-zip-code").text());

  if (twoDayZips.includes(curZip) && firstDeliveryDate < twoDaysOut || fiveDayZips.includes(curZip) && firstDeliveryDate < fiveDaysOut) {
...
var anotherInterval = setInterval(function () {
  if (typeof window.jQuery !== "undefined" && typeof window.server !== "undefined") {
