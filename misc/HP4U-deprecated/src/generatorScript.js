...
class Store {
  ...
  async getBestSellers() {
    ...
    while (data.response.docs.length > 0) {
      const chunk = data.response.docs.splice(0, 4);
  ...
  generateLinks() {
    ...
    if (this.titleCorpusSorted.length > sectionLength) {
      ...
      this.searchLinks = arr.map((item) => `<li><a href="https://www.livingspaces.com/search?term=${item}" title="${item}">${item.replace(/^\w/, (c) => c.toUpperCase())}</a></li>`);
    ...
    if (this.relatedItems) {
      for (const key in this.relatedItems) {
        if (this.relatedItems.hasOwnProperty(key)) {
          this.itemHtml.push(
            this.relatedItems[key].map((item) => `<div class='col-xs-6 col-md-3'><a href='https://www.livingspaces.com/${item.pid}'><img class='img-responsive' src='${item.thumb_image}' alt='${item.title}'><p class='item-text'>${item.title}</p><p class='price-text'>$${item.sale_price}</p></a></div>`),
          );
        }
      }
    } else {
      for (i = 0; i < this.bestSellers.length; i++) {
        this.itemHtml.push(
          this.bestSellers[i].map((item) => `<div class='col-xs-6 col-md-3'><a href='https://www.livingspaces.com/${item.pid}'><img class='img-responsive' src='${item.thumb_image}' alt='${item.title}'><p class='item-text'>${item.title}</p><p class='price-text'>$${item.sale_price}</p></a></div>`),
        );
      }
    }
